...
import {
  ...
  import { deliveryAvailableRequest } from '../delivery/reducer/SalesReducer';
...
function ReleaseProcess() {
    let today = moment(new Date()).format('yyyy-MM-DD');
      ...
      const [newModalOpen, setNewModalOpen] = useState(false); // 새로운 모달 상태
      const [transportList, setTransportList] = useState([]); // 운송수단 목록
      const [selectedTransport, setSelectedTransport] = useState("운송수단"); // 버튼에 표시할 운송수단 이름
      ...
      const column2 = [
        ...
        { headerName: '출고완료여부', field: 'releaseCompletionStatus' },
    ...
    const conditionChange = (e) => {
      ...
      const fetchTransportList = async () => {
        try {
          const response = await axios.get("http://localhost:9102/compinfo/codedetail/driver");

          const detailCodeList = response.data?.detailCodeList || [];
          console.log(detailCodeList);
          setTransportList(detailCodeList); // 운송수단 데이터 설정
          setNewModalOpen(true); // 모달 열기
        } catch (error) {
          console.error("운송수단 데이터 조회 중 오류 발생:", error);
        }
      };

      // 운송수단 선택 시 처리
      const handleTransportSelect = (name:any) => {
        setSelectedTransport(name); // 선택된 운송수단 이름 설정
        setNewModalOpen(false); // 모달 닫기
      };
        ...
        const contractDetailData = async () => {
          ...
          const release = async () => {
            ...
            try {
              ...
              const transportMethod = selectedTransport;

              if (!transportMethod || transportMethod === "운송수단") {
                Swal.fire({
                  icon: 'error',
                  title: '운송수단을 선택해주세요.'
                });
              ...
              await axios.get('http://localhost:9102/sales/release', {
                params: {
    ...
    return (
        <Page title="출고">
      <MainCard content={false} title="출고">
        ...
        <div style={{ marginRight: '1vh', marginTop: '1vh', marginLeft: '1vh', float: 'right' }}>
          ...
          <Button
            ...
            color="secondary"
            style={{ marginRight: "1vh", marginTop: "1vh" }}
            onClick={fetchTransportList} // 버튼 클릭 시 모달 열기
          >
            {selectedTransport || "운송수단"} {/* 선택된 운송수단이 있으면 해당 이름으로 변경 */}
          </Button>
         
          <Button variant="contained" color="secondary" style={{ marginTop: '1vh', marginRight: '1vh' }} onClick={contractDetailData}>
            수주내역 조회
          </Button>
          <Button variant="contained" color="secondary" style={{ marginTop: '1vh', marginRight: '1vh' }} onClick={release}>
            출고
          </Button>
        </div>
        ...
        <React.Fragment>
          ...
          <Modal
            open={newModalOpen}
            onClose={() => setNewModalOpen(false)} // 모달 닫기
            aria-labelledby="transport-modal-title"
            aria-describedby="transport-modal-description"
          >
            <Box
              sx={{
                position: "absolute",
            ...
            >
              <h2 id="transport-modal-title" style={{ color: "white", marginBottom: "10px" }}>
                운송수단 목록
              </h2>

              {/* DataGrid에 transportList 데이터 표시 */}
              <DataGrid
                rows={transportList.map((item, index) => ({
                  id: index + 1, // 고유 ID
                  ...item,
                }))}
                columns={[
                  { field: "detailCode", headerName: "Detail Code", flex: 1 },
                  { field: "detailCodeName", headerName: "Detail Code Name", flex: 1 },
                ]}
                pageSize={10}
                rowsPerPageOptions={[10]}
                getRowId={(row) => row.id} // 행 고유 ID
                sx={{
                  color: "white", // 글자 색상 흰색
                  "& .MuiDataGrid-cell": {
                    color: "white", // 셀 글자 색상
                ...
                onRowClick={(params) => {
                  setSelectedTransport(params.row.detailCodeName); // 선택된 운송수단 이름 설정
                  setNewModalOpen(false); // 모달 닫기
                }}
              />
...
ReleaseProcess.getLayout = function getLayout(Page) {
