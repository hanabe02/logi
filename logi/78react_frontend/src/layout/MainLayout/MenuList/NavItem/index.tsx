...
const NavItem = ({ item, level, parentId, isParents = false }: NavItemProps) => {
  ...
  const itemHandler = (id: string) => {
    console.log("page 가 변경되었습니다.");
    dispatch(activeItem([id])); // 선택된 메뉴 항목 업데이트트
    console.log(id);
    if (matchesSM) dispatch(openDrawer(false)); 
    // 화면 크기에 따라 사이드바 닫기
    dispatch(activeID(parentId)); // 부모 메뉴 id 업데이트 
  };
  ...
  return (
    <>
      {layout === LAYOUT_CONST.VERTICAL_LAYOUT || (layout === LAYOUT_CONST.HORIZONTAL_LAYOUT && matchDownMd) ? (
        <ListItemButton
          component={Link}
          // Link 컴포넌트로 동작하도록 만들어, react router를 통해 클라이언트 사이드 라우팅이 실행된다.
          ...
          // 메뉴 항목을 클릭하면 itemHandler 가 실행되어 '상태 관리 로직' 이 실행행
        >
